        ...
        If Tick > tmr1000 Then
            ...
            If IsPlaying(i) Then
                ...
                If TempPlayer(i).tmpGuildInviteSlot > 0 Then
                    If Tick > TempPlayer(i).tmpGuildInviteTimer Then
                        If GuildData(TempPlayer(i).tmpGuildInviteSlot).In_Use = True Then
                            PlayerMsg i, "Guild invitation timed out for " & GuildData(TempPlayer(i).tmpGuildInviteSlot).Guild_Name & ".", BrightRed
                ...
                If GetInactiveTime(i) >= 900 And GetPlayerAccess_Mode(i) < ADMIN_MONITOR Then
                    AlertMsg i, "you have been kicked for inactivity"
                End If
                                        ...
                                        If DistanceX <= N And DistanceY <= N Then
                                            If NPC(npcnum).Behaviour = NPC_BEHAVIOUR_ATTACKONSIGHT Or GetPlayerPK(i) = YES Then
                                                If Len(Trim$(NPC(npcnum).AttackSay)) > 0 Then
                                                    ...
                                                    Call SendActionMsg(mapnum, NPC(npcnum).AttackSay, SayColor, 1, MapNpc(mapnum).NPC(X).X * 32, MapNpc(mapnum).NPC(X).Y * 32)
                                                End If
...
Private Sub HandleShutdown()
    ...
    If Secs <= 0 Then
        Call GlobalMsg("Server Down.", BrightRed)
        Call DestroyServer
        'Call RestartServer
    End If
