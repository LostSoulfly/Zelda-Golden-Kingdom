...
'spawn pet on new map
    ...
    If map(PlayerMap).moral = MAP_MORAL_SAFE Or map(PlayerMap).moral = MAP_MORAL_PACIFIC Then PetDisband index, OldMap, True: PlayerMsg index, "Your pet is not allowed on this map.", White: Exit Sub
...
Public Sub SpawnPet(ByVal index As Long, ByVal mapnum As Long)
    ...
    If UntilTime <= 0 Then
        Call PlayerMsg(index, "You can't even summon your pet!, miss" & Round(Abs(UntilTime) / 1000, 0) & "seconds!", BrightRed)
        Exit Sub
    End If
    ...
    End Select
        ...
        If map(PlayerMap).moral = MAP_MORAL_SAFE Or map(PlayerMap).moral = MAP_MORAL_PACIFIC Then PetDisband index, PlayerMap, True: PlayerMsg index, "Your pet is not allowed on this map.", White: Exit Sub
...
If TempPlayer(index).TargetType <> TARGET_TYPE_NPC Then
    Call PlayerMsg(index, "You don't have...no... NPC fixed", BrightRed)
    Exit Sub
End If
...
If Not (GetPlayerTamePoints(index, Pet(PetIndex).npcnum) >= Pet(PetIndex).TamePoints) Then
    If GetPlayerAccess(index) < ADMIN_CREATOR Then
        Call PlayerMsg(index, "You don't have enough tame points for this pet!", BrightRed)
        Exit Sub
    End If
End If
...
Call PlayerMsg(index, Trim$(NPC(Pet(PetIndex).npcnum).Name) & " has joined your party!", BrightGreen)
...
Sub CheckPlayerPetLevelUp(ByVal index As Long)
    ...
    Do While GetPlayerPetExp(index) >= GetPlayerPetNextLevel(index)
        ...
        'PlayerMsg index, PointsWon & "points won!", cyan
    ...
    If level_count > 0 Then
        If level_count = 1 Then
            'singular
            PlayerMsg index, "Your pet has reached level " & level_count & "!", Brown
        Else
            'plural
            PlayerMsg index, "Your pet has gained " & level_count & " levels!", Brown
        End If
        'SendEXP index
...
Call PlayerMsg(index, "You abandon your" & Trim$(NPC(Pet(player(index).Pet(PetSlot).NumPet).npcnum).Name), Yellow)
Call ResetPlayerPetSlot(index, PetSlot)
