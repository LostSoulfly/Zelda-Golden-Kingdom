...
Sub CheckDoor(ByVal index As Long, ByVal X As Long, ByVal Y As Long)
                    ...
                    If Doors(Door_Num).UnlockType = 0 Then
                        For i = 1 To MAX_INV
                            ...
                            If Doors(Door_Num).key = key Then
                                ...
                                PlayerMsg N, "Something has been unlocked..", Cyan
                                SendMapSound index, GetPlayerX(index), GetPlayerY(index), SoundEntity.seSwitch, 1
                                ' End
                                Exit Sub
                            End If
                        Next
                        PlayerMsg index, "You don't have the right key for this door..", Cyan
                    ElseIf Doors(Door_Num).UnlockType = 1 Then
                        If Not TempTile(GetPlayerMap(index)).Door(TempDoorNum).state Then
                            PlayerMsg index, "You didn't press the right switch for this door..", Cyan
                        End If
                    ElseIf Doors(Door_Num).UnlockType = 2 Or Doors(Door_Num).UnlockType = 3 Then
                        PlayerMsg index, "The door is closed.", Cyan
                    End If
                    
                Else
                    PlayerMsg index, "The door is already open.", Cyan
                End If
                ...
                If Not IsDoorOpened(GetPlayerMap(index), TempDoorNum) Then 'checking if switch is activated
                    ...
                    If (Doors(Door_Num).Switch) > 0 Then
                        ...
                        If Switch > 0 Then
                            ...
                            SendMapSound index, GetPlayerX(index), GetPlayerY(index), SoundEntity.seSwitch, 1
                        End If
                        MapMsg GetPlayerMap(index), "A switch has been activated.", Cyan
                ...
                Else
                    ...
                    If (Doors(Door_Num).Switch) > 0 Then
                        ...
                        If Switch > 0 Then
                            SetAllMapDoorNum GetPlayerMap(index), Doors(Door_Num).Switch
                        End If
                        PlayerMsg index, "The switch has been deactivated.", Cyan
                        SendMapSound index, GetPlayerX(index), GetPlayerY(index), SoundEntity.seSwitch, 1
                    End If
                End If
            End If
    ...
    'PlayerMsg index, "running the tile_type_key attack open test.", White, True
        Door_Num = map(GetPlayerMap(index)).Tile(X, Y).Data1
            ...
            If KeyToOpen > 0 Then
                If HasItem(index, ItemNum) Then
                    ...
                    If map(GetPlayerMap(index)).Tile(X, Y).Data2 = 1 Then
                        ...
                        Call PlayerMsg(index, Trim$(item(ItemNum).Name) & " was destroyed!", Yellow)
                    End If
                'SendPlayerSound index, GetPlayerX(index), GetPlayerY(index), SoundEntity.seItem, ItemNum
                Else
                    Call PlayerMsg(index, "You do not have " & Trim$(item(ItemNum).Name) & ".", Yellow)
                End If
            End If
        End If
    End If
