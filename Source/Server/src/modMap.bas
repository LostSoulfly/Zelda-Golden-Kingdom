...
Public Function GetMapData(ByRef MapT As MapRec) As Byte()
    ...
    With MapT
        ...
        'Buffer.WriteConstString GetTranslation(.Name)
...
Public Function GetMapDataT(ByRef MapT As MapRec) As Byte()
    Dim Buffer As clsBuffer
    Set Buffer = New clsBuffer
    With MapT
...
AddLog2 "CheckMapsConnectedTo: Checking map " & Trim$(map(mapnum).Name) & " (" & mapnum & ") for linkage from other maps.", "MAP_CHECK.log"

    For i = 1 To MAX_MAPS
        With map(i)

        If .left = mapnum Then
            AddLog2 "CheckMapsConnectedTo: Left of " & Trim$(map(i).Name) & " (" & i & ") connects to " & mapnum, "MAP_CHECK.log"
            CheckMapsConnectedTo = True
        End If
        
        If .right = mapnum Then
            AddLog2 "CheckMapsConnectedTo: Right of " & Trim$(map(i).Name) & " (" & i & ") connects to " & mapnum, "MAP_CHECK.log"
            CheckMapsConnectedTo = True
        End If
        
        If .Up = mapnum Then
            AddLog2 "CheckMapsConnectedTo: Up of " & Trim$(map(i).Name) & " (" & i & ") connects to " & mapnum, "MAP_CHECK.log"
            CheckMapsConnectedTo = True
        End If
        
        If .Down = mapnum Then
            AddLog2 "CheckMapsConnectedTo: Down of " & Trim$(map(i).Name) & " (" & i & ") connects to " & mapnum, "MAP_CHECK.log"
            CheckMapsConnectedTo = True
        End If
        ...
        For X = 0 To .MaxX
            For Y = 0 To .MaxY
                If .Tile(X, Y).Type = TILE_TYPE_WARP Then
                    ...
                    AddLog2 "Map " & Trim$(map(i).Name) & " (" & i & ") connects to " & mapnum & " by warp from X: " & X & " Y: " & Y & _
...
Public Sub SetServerMapData(ByVal mapnum As Long, ByRef Data() As Byte)
    ...
    With map(mapnum)
        If Buffer.ReadConstString(3, False) = "v.2" Then
            newVer = True
            Buffer.MoveReadHead 3
        End If
  
        .Name = Buffer.ReadConstString(NAME_LENGTH)
        If newVer = True Then TranslatedName = Buffer.ReadConstString(NAME_LENGTH)
        'If newVer = False Then Buffer.MoveReadHead NAME_LENGTH
