        ...
        If IsGameRunning(GameIndex) Then
            ...
            Case CaptureTheFlag
                ...
                If Flag > 0 Then
                    ...
                    GameMsg GameIndex, GetPlayerName(index) & " has disconnected and the flag was returned to base.", GetFlagColour(GetPlayerFlag(index))
                End If
...
Sub CheckIfGameFinished(ByVal GameIndex As Byte)
    ...
    Case CaptureTheFlag
        ...
        If .CTFInfo.Score(i) >= GetCTFMaxScore Then
            If .Teams(i) > 0 Then
                GlobalMsg "The team of " & GetPlayerName(Teams(.Teams(i)).Representant) & " has won the game!", BrightGreen
                AddLog 0, "Winner: " & GetPlayerName(Teams(.Teams(i)).Representant), ADMIN_LOG
                AdminMsg "Winner: " & GetPlayerName(Teams(.Teams(i)).Representant), BrightGreen
            End If
            Dim j As Byte
            For j = 1 To .NTeams
                If .Teams(j) > 0 Then
                        ...
                        If CTFIsInRange(index, i) Then
                            If GetPlayerMap(index) = GetPlayerMap(i) Then
                                ...
                                If Flag > 0 Then
                                    ...
                                    GameMsg GameIndex, GetPlayerName(index) & " has touched " & GetPlayerName(i) & " and returned the flag to its base!", GetFlagColour(GetPlayerFlag(index))
                                End If
        ...
        If IsGameRunning(GameIndex) Then
            ...
            If RivalFlag > 0 Then
                If Not IsFlagTaken(GameIndex, RivalFlag) Then
                    ...
                    GameMsg GameIndex, GetPlayerName(index) & " has taken the flag!", GetFlagColour(TeamFlag)
                    DisplayFlag GameIndex, RivalFlag, False
                Else
                    PlayerMsg index, "There is no flag here", BrightRed
                End If
            ElseIf OnOwnFlag(index, TeamFlag) Then
                If Not IsFlagTaken(GameIndex, TeamFlag) Then 'our flag cannot be out of base
                    ...
                    If CarryingFlag > 0 Then
                        ...
                        GameMsg GameIndex, GetPlayerName(index) & " has deposited the flag!", GetFlagColour(TeamFlag)
                        GameMsg GameIndex, "The team of " & GetPlayerName(Teams(PlayerTeam).Representant) & " has gained a point. They now have: " & GetFlagPoints(GameIndex, TeamFlag), GetFlagColour(TeamFlag)
                    Else
                        PlayerMsg index, "This is your base!", BrightRed
                    End If
                Else
                    PlayerMsg index, "Your flag is not here..", BrightRed
                End If
            End If
        End If
    End If
