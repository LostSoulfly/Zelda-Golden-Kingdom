...
Public Sub SpawnNpc(ByVal mapnpcnum As Long, ByVal mapnum As Long, Optional ByVal SetX As Long, Optional ByVal SetY As Long, Optional ByVal npcnum As Long = 0, Optional nearOwner As Boolean = False)
    ...
    '    If PetOwner > 0 Then
    '        PlayerMsg PetOwner, "Your pet has followed you.", White
    '    End If
    'End If
    ...
    If partynum > 0 Then
        ...
        If Party(partynum).MemberCount > 2 Then
            ...
            If Party(partynum).Leader = index Then
                ...
                For i = 1 To MAX_PARTY_MEMBERS
                    If Party(partynum).Member(i) > 0 And Party(partynum).Member(i) <> index Then
                        ...
                        PartyMsg partynum, GetPlayerName(i) & "he is now the leader of the team.", Cyan
                        Exit For
                    End If
                Next
                ' leave party
                PartyMsg partynum, GetPlayerName(Party(partynum).Leader) & "he has left the team", BrightRed
                ' remove from array
                For i = 1 To MAX_PARTY_MEMBERS
            ...
            Else
                ...
                PartyMsg partynum, GetPlayerName(index) & "he has left the team", BrightRed
                ' remove from array
                For i = 1 To MAX_PARTY_MEMBERS
                    If Party(partynum).Member(i) = index Then
                        Party(partynum).Member(i) = 0
        ...
        Else
            ...
            PartyMsg partynum, "Team disbanded.", BrightRed
...
Dim partynum As Long, i As Long
    ...
    If TempPlayer(targetPlayer).inParty > 0 Then
        ...
        PlayerMsg index, "This player is already part of the team.", BrightRed
        'exit out early
        Exit Sub
    End If
        ...
        If Party(partynum).Leader = index Then
            ...
            For i = 1 To MAX_PARTY_MEMBERS
                If Party(partynum).Member(i) = 0 Then
                    ...
                    ' let them know
                    PlayerMsg index, "Invitation has been sent.", Pink
                    Exit Sub
                End If
            Next
            ' no room
            PlayerMsg index, "The team is full.", BrightRed
            Exit Sub
        Else
            ' not the leader
            PlayerMsg index, "You are not the leader of the team.", BrightRed
            Exit Sub
        End If
    Else
...
Dim partynum As Long, i As Long, X As Long
    ...
    If TempPlayer(index).inParty > 0 Then
        ...
        For i = 1 To MAX_PARTY_MEMBERS
            ...
            Next
                ...
                PartyMsg partynum, GetPlayerName(targetPlayer) & "he has joined the team.", Pink
                ' add them in
        ...
        PlayerMsg index, "The team is full.", BrightRed
        PlayerMsg targetPlayer, "The team is full.", BrightRed
    ...
    Else
        ...
        PartyMsg partynum, "Team created.", BrightGreen
        PartyMsg partynum, GetPlayerName(index) & "he has joined the team.", Pink
        PartyMsg partynum, GetPlayerName(targetPlayer) & "he has joined the team.", Pink
...
Public Sub Party_InviteDecline(ByVal index As Long, ByVal targetPlayer As Long)
    PlayerMsg index, GetPlayerName(targetPlayer) & "he has refused to join the team.", BrightRed
    PlayerMsg targetPlayer, "You have refused to join the team.", BrightRed
    ' clear the invitation
    TempPlayer(targetPlayer).partyInvite = 0
End Sub
    ...
    If i > 0 And i <> index Then
        If CanPlayerAttackPlayer(index, i, True) = True Then
            ...
            If CanPlayerDodge(i) Then
                SendActionMsg mapnum, "Dodged!", Pink, 1, (GetPlayerX(i) * 32), (GetPlayerY(i) * 32)
                Exit Sub
            End If
            ...
            Else
                Call PlayerMsg(index, "You missed!", BrightRed)
            End If
        Else
            'PlayerMsg index, "impossible to attack", BrightRed
        End If
        ...
        'If x = mapnpc(mapnum).NPC(i).x And y = mapnpc(mapnum).NPC(i).y Then
            ...
            If CanPlayerAttackNpc(index, i, True) Then
                If CanNpcDodge(index, npcnum) Then
                    SendActionMsg mapnum, "Dodged!", Pink, 1, (MapNpc(mapnum).NPC(i).X * 32), (MapNpc(mapnum).NPC(i).Y * 32)
                    Exit Sub
                End If
                If CanNpcParry(index, npcnum) Then
                    SendActionMsg mapnum, "Parried!", Pink, 1, (MapNpc(mapnum).NPC(i).X * 32), (MapNpc(mapnum).NPC(i).Y * 32)
...
Public Sub ScriptTilePresses(ByVal index As Long, ByVal scriptnum As Long)
    If scriptnum > 0 Then
        ...
        Case 2
            ...
            MapMsg GetPlayerMap(index), "The blocks have returned to their position", BrightGreen
        Case 3
            Call Exchange(index)
        ...
        'Call PlayerMsg(index, "You have stepped on the tile n script." & Scriptnum, 15)
    End If
...
Case 0 'sub-vital
    ...
    If .Data1 = Vitals.HP Then
        If vital >= GetPlayerVital(index, Vitals.HP) Then
            Call MapMsg(GetPlayerMap(index), GetPlayerName(index) & " has died to an obstacle!", BrightRed)
...
PlayerMsg index, "Change made", BrightGreen
    ...
    For j = 0 To X
        For k = 0 To Y
            If map(i).Tile(j, k).Type = TILE_TYPE_ITEM Then
                PlayerMsg index, "Map:" & " " & i & ", X: " & j & ", Y: " & k & ", " & "item number:" & " " & map(i).Tile(j, k).Data1 & "(" & map(i).Tile(j, k).Data2 & ") : " & Trim$(item(map(i).Tile(j, k).Data1).Name), White
            End If
        Next
    Next
Next
...
AdminMsg "IP: " & IP & "added correctly", BrightGreen
...
On Error GoTo oops
    ...
    Case FindItem
        ...
        For i = 1 To MAX_ITEMS
            If InStr(1, LCase$(GetItemName(i)), ItemName) > 0 Then
                PlayerMsg index, GetItemName(i) & " #" & i, BrightGreen
            End If
        Next
        PlayerMsg index, "-End of Items-", BrightGreen, True
    ...
    Case FindNPC
        ...
        For i = 1 To MAX_NPCS
            If InStr(1, LCase$(GetNPCName(i)), NPCName) > 0 Then
                PlayerMsg index, GetNPCName(i) & ": " & i, BrightGreen
            ElseIf InStr(1, LCase$(NPC(i).Name), NPCName) > 0 Then
                PlayerMsg index, GetNPCName(i) & " (" & Trim$(NPC(i).Name) & "): " & i, BrightGreen
            End If
        Next
        
        PlayerMsg index, "-End of NPCs-", BrightGreen, True
    ...
    Case FindMAP
        ...
        For i = 1 To MAX_MAPS
            If InStr(1, LCase$(map(i).Name), LCase(strSearch)) > 0 Then
                 PlayerMsg index, Trim$(map(i).Name) & " #" & i, BrightGreen, True
            ElseIf InStr(1, LCase$(map(i).Name), LCase(strSearch)) > 0 Then
                PlayerMsg index, Trim$(map(i).Name) & " #" & i, BrightGreen, True
            End If
        Next i
        PlayerMsg index, "-End of Maps-", BrightGreen, True
    ...
    Case InspectPlayer
        ...
        i = FindPlayer(s(2))
        If i > 0 Then
            PlayerMsg index, "INV:", BrightGreen
            For j = 1 To MAX_INV
            
                ItemNum = GetPlayerInvItemNum(i, j)
                If ItemNum > 0 Then
                    If isItemStackable(ItemNum) Then
                        PlayerMsg index, j & ": " & ItemNum & ", " & GetItemName(ItemNum) & ", " & GetPlayerInvItemValue(i, j), BrightGreen
                    Else
                        PlayerMsg index, j & ": " & ItemNum & ", " & GetItemName(ItemNum), BrightGreen
                    End If
                End If
            Next
            
            PlayerMsg index, "BANK:", BrightGreen
            For j = 1 To MAX_BANK
                ItemNum = GetPlayerBankItemNum(i, j)
                If ItemNum > 0 Then
                    PlayerMsg index, j & ": " & ItemNum & ", " & GetItemName(ItemNum) & ", " & GetPlayerBankItemValue(i, j), BrightGreen
                End If
            Next
            ...
            For j = 1 To MAX_PLAYER_SPELLS
                ...
                If spellnum > 0 Then
                    PlayerMsg index, j & ": " & spellnum, BrightGreen
                End If
            Next
        End If
    ...
    Case ViewKillPoints
        ...
        For i = 1 To Player_HighIndex
            If IsPlaying(i) Then
                PlayerMsg index, GetPlayerName(i) & ": " & GetPlayerKillPoints(i, GetPlayerPK(i)), GetColorByJustice(GetPlayerPK(i))
            End If
        Next
            ...
            For X = 0 To map(i).MaxX
                For Y = 0 To map(i).MaxY
                    If GetTileType(i, X, Y) = TILE_TYPE_SCRIPT Then
                        PlayerMsg index, "map: " & i & ", " & X & ", " & Y & ", Data: " & map(i).Tile(X, Y).Data1, Yellow
                    End If
                Next
            Next
        Next
            ...
            For X = 0 To map(i).MaxX
                For Y = 0 To map(i).MaxY
                    If GetTileType(i, X, Y) = TILE_TYPE_SHOP Then
                        PlayerMsg index, "map: " & i & ", " & X & ", " & Y & ", Data1: " & map(i).Tile(X, Y).Data1 & ", Data2: " & map(i).Tile(X, Y).Data2 & ", Data3: " & map(i).Tile(X, Y).Data3, Yellow, True
                    End If
                Next
            Next
        Next
    ...
    PlayerMsg index, "Shop report started..", White: DoEvents
                ...
                If .Tile(X, Y).Type = TILE_TYPE_SHOP Then
                    If .Tile(X, Y).Data1 > 0 Then ' tile has shop?
                        If Len(Trim$(Shop(X).Name)) > 0 Then ' Shop has name?
                            PlayerMsg index, "Map " & Trim$(map(i).Name) & " (" & i & ") has shop #" & .Tile(X, Y).Data1 & " (" & Trim$(Shop(.Tile(X, Y).Data1).Name) & ")" & " at X: " & X & " Y: " & Y, White, True
                        Else
                            PlayerMsg index, "Map " & Trim$(map(i).Name) & " (" & i & ") has shop #" & .Tile(X, Y).Data1 & " (NO NAME)" & " at X: " & X & " Y: " & Y, White, True
                        End If
                    ...
                    Else
                        PlayerMsg index, "Map " & Trim$(map(i).Name) & " (" & i & ") has TILE_TYPE_SHOP " & " at X: " & X & " Y: " & Y, White, True
                    End If
                End If
            Next
        Next
    ...
    For ii = 1 To UBound(ShopsUnused)
        If Not ShopsUnused(ii) = vbNullString Then
            PlayerMsg index, "Shop " & Trim$(Shop(ii).Name) & "(" & ii & ") is not used anywhere!", White, True
        End If
    Next ii
    
PlayerMsg index, "-end of shopreport-", White
    ...
    Case TurnGlobalChat
        ...
        If MinAccess >= 0 And MinAccess <= ADMIN_CREATOR Then
            ...
            AdminMsg "Global Chat disabled for those with access less than: " & MinAccess, BrightRed
        End If
    ...
    End If
        ...
        PlayerMsg index, "Map report for " & mapnum, White
        If Not l = 0 Then PlayerMsg index, "Map Left: " & Trim$(map(l).Name) & "(" & l & ")", White
        If Not r = 0 Then PlayerMsg index, "Map Right: " & Trim$(map(r).Name) & "(" & r & ")", White
        If Not u = 0 Then PlayerMsg index, "Map Up: " & Trim$(map(u).Name) & "(" & u & ")", White
        If Not d = 0 Then PlayerMsg index, "Map Down: " & Trim$(map(d).Name) & "(" & d & ")", White
        PlayerMsg index, "~~~~~~~~~~~~~~~~~", White
    ...
    DoEvents
        ...
        If .left = mapnum Then
            PlayerMsg index, "Map " & Trim$(map(i).Name) & " (" & i & ") connects to the left.", White
        End If
        
        If .right = mapnum Then
            PlayerMsg index, "Map " & Trim$(map(i).Name) & " (" & i & ") connects to the right.", White
        End If
        
        If .Up = mapnum Then
            PlayerMsg index, "Map " & Trim$(map(i).Name) & " (" & i & ") connects to the top.", White
        End If
        
        If .Down = mapnum Then
            PlayerMsg index, "Map " & Trim$(map(i).Name) & " (" & i & ") connects to the bottom.", White
        End If
            ...
            For Y = 0 To .MaxY
                If .Tile(X, Y).Type = TILE_TYPE_WARP Then
                    If CLng(.Tile(X, Y).Data1) = mapnum Then
                         PlayerMsg index, "Map " & Trim$(map(i).Name) & " (" & i & ") connects by warp from X: " & X & " Y: " & Y & _
                         " to X: " & .Tile(X, Y).Data2 & " Y: " & .Tile(X, Y).Data3, White
                    End If
                End If
            Next
        Next
        End With
    Next
PlayerMsg index, "-end of mapreport-", White
    ...
    Case GiveItem
        ...
        Case 2 '3 long
            If IsNumeric(s(2)) Then
                ...
                SendActionMsg GetPlayerMap(index), item(CLng(s(2))).Name, White, 1, (GetPlayerX(index) * 32), (GetPlayerY(index) * 32)
            Else: GoTo howto
            End If
        ...
        Case 3 '4 long
            If IsNumeric(s(2)) Then
                If IsNumeric(s(3)) Then
                    ...
                    SendActionMsg GetPlayerMap(index), s(3) & " " & item(CLng(s(2))).Name, White, 1, (GetPlayerX(index) * 32), (GetPlayerY(index) * 32)
                Else
            ...
            Else
                ...
                If i > 0 Then 'found a player.
                    If IsNumeric(s(3)) Then
                        ...
                        SendActionMsg GetPlayerMap(i), item(CLng(s(3))).Name, White, 1, (GetPlayerX(index) * 32), (GetPlayerY(index) * 32)
                    Else
                        GoTo howto
                    End If
                Else
                    PlayerMsg index, "Player not found.", Yellow
                End If
            End If
        ...
        Case 4 '5 long
            ...
            If IsNumeric(s(3)) And IsNumeric(s(4)) Then
                ...
                If i > 0 Then 'found a player.
                    ...
                    SendActionMsg GetPlayerMap(i), s(4) & " " & item(CLng(s(3))).Name, White, 1, (GetPlayerX(index) * 32), (GetPlayerY(index) * 32)
                Else
                    PlayerMsg index, "Player not found.", Yellow
                End If
            Else
        ...
        Case Else
howto:
        PlayerMsg index, "-GiveItem HowTo- (Only works for non-numeric names!)", White, True
        PlayerMsg index, "[/cmd giveitem 123] will add item 123 to your inventory.", White, True
        PlayerMsg index, "[/cmd giveitem 123 10] will add 10 of item 123 to your inventory.", White, True
        PlayerMsg index, "[/cmd giveitem Dragoon 123] will add item 123 to Dragoon's inventory.", White, True
        PlayerMsg index, "[/cmd giveitem Dragoon 123 5] will add 5 of item 123 to Dragoon's inventory.", White, True
        End Select
    End Select
...
oops:
PlayerMsg index, "Error: " & Err.Number & ": " & Err.Description, BrightGreen, True
