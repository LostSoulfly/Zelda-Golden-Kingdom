...
Dim Damage As Long
    ...
    If CanPlayerAttackNpc(index, MapNPCNum) Then
        ...
        If CanNpcParry(index, NPCNum) Then
            SendActionMsg mapnum, "Blocked!", Pink, 1, (mapnpc(mapnum).NPC(MapNPCNum).x * 32), (mapnpc(mapnum).NPC(MapNPCNum).y * 32)
            Exit Sub
        End If
...
Dim Buffer As clsBuffer
    ...
    If CanNpcAttackPlayer(MapNPCNum, index) Then
        ...
        Else
            SendActionMsg mapnum, "Averted!", Cyan, 1, (GetPlayerX(index) * 32), (GetPlayerY(index) * 32)
        End If
    End If
End Sub
...
Dim Damage As Long
    ...
    If CanPlayerAttackPlayer(attacker, victim) Then
        ...
        If CanPlayerParry(victim, attacker) Then
            SendActionMsg mapnum, "Blocked!", Pink, 1, (GetPlayerX(victim) * 32), (GetPlayerY(victim) * 32)
            Exit Sub
        End If
...
Function CanPlayerAttackPlayer(ByVal attacker As Long, ByVal victim As Long, Optional ByVal IsSpell As Boolean = False, Optional ByVal IsProjectile As Boolean = False) As Boolean
    ...
    If Not Map(GetPlayerMap(attacker)).Moral = MAP_MORAL_NONE Then
        If IsPlayerNeutral(victim) Then
            Call PlayerMsg(attacker, "This is a safe area!", BrightRed)
            Exit Function
        End If
    End If
    ...
    If GetPlayerAccess(attacker) > ADMIN_MONITOR Then
        Call PlayerMsg(attacker, "Administrators cannot attack other users.", BrightBlue)
        Exit Function
    End If

    ' Check to make sure the victim isn't an admin
    If GetPlayerAccess(victim) > ADMIN_MONITOR Then
        Call PlayerMsg(attacker, "You can't attack a" & GetPlayerName(victim) & "!", BrightRed)
        Exit Function
    End If

    ' Make sure attacker is high enough level
    If GetPlayerLevel(attacker) < 10 Then
        Call PlayerMsg(attacker, "You're under level 10, you can't attack anyone yet!", BrightRed)
        Exit Function
    End If

    ' Make sure victim is high enough level
    If GetPlayerLevel(victim) < 10 Then
        Call PlayerMsg(attacker, "ï¿½" & GetPlayerName(victim) & "is under level 10, you can't attack him yet", BrightRed)
        Exit Function
    End If
    ...
    If Player(attacker).GuildFileId > 1 Then
        If Player(attacker).GuildFileId = Player(victim).GuildFileId Then
            Call PlayerMsg(attacker, "¡¡" & GetPlayerName(victim) & "he is a member of your Guild!", BrightRed)
        Exit Function
        End If
    End If
...
Sub PlayerAttackPlayer(ByVal attacker As Long, ByVal victim As Long, ByVal Damage As Long, Optional ByVal Spellnum As Long = 0)
    ...
    If Damage >= GetPlayerVital(victim, Vitals.HP) Then
        ...
        ' Player is dead
        Call GlobalMsg(GetPlayerName(victim) & "he was killed by" & GetPlayerName(attacker), BrightRed)
        ' Calculate exp to give attacker
        ...
        If Exp = 0 Then
            Call PlayerMsg(victim, "You have not lost any experience.", BrightRed)
            Call PlayerMsg(attacker, "You received no experience.", BrightBlue)
        Else
            Call SetPlayerExp(victim, GetPlayerExp(victim) - Exp)
            SendEXP victim
            Call PlayerMsg(victim, "You have lost" & Exp & "of experience!", BrightRed)
        ...
        'If IsPlayerNeutral(victim) Then
            'If IsPlayerNeutral(attacker) Then
                ...
                'Call GlobalMsg("ï¿½" & GetPlayerName(attacker) & " has become a murderer!", BrightRed)
            'End If

        'Else
            'Call GlobalMsg("ï¿½" & GetPlayerName(victim) & " has paid the price for being a murderer!", BrightRed)
        'End If
...
Public Sub BufferSpell(ByVal index As Long, ByVal spellslot As Long)
    ...
    If LevelReq > GetPlayerLevel(index) Then
        Call PlayerMsg(index, "You need the LVL" & LevelReq & "to use this skill.", BrightRed)
        Exit Sub
    End If
    ...
    If AccessReq > GetPlayerAccess(index) Then
        Call PlayerMsg(index, "You need to be an admin.", BrightRed)
        Exit Sub
    End If
    ...
    If ClassReq > 0 Then ' 0 = no req
        If ClassReq <> GetPlayerClass(index) Then
            Call PlayerMsg(index, "Just the Class" & CheckGrammar(Trim$(Class(ClassReq).Name)) & "you can use this magic.", BrightRed)
            Exit Sub
        End If
    End If
...
Public Sub CastSpell(ByVal index As Long, ByVal spellslot As Long, ByVal target As Long, ByVal targetType As Byte)
    ...
    ' Make sure they are the right level
    If LevelReq > GetPlayerLevel(index) Then
        Call PlayerMsg(index, "You must be level" & LevelReq & "to use this skill.", BrightRed)
        Exit Sub
    End If
    ...
    ' make sure they have the right access
    If AccessReq > GetPlayerAccess(index) Then
        Call PlayerMsg(index, "You must be an administrator to use this skill.", BrightRed)
        Exit Sub
    End If
    ...
    If ClassReq > 0 Then ' 0 = no req
        If ClassReq <> GetPlayerClass(index) Then
            Call PlayerMsg(index, "Solo " & (Class(ClassReq).Name) & "you can use this skill.", BrightRed)
            Exit Sub
        End If
    End If
...
Dim NPCNumAttacker As Long, NPCNumVictim As Long, blockAmount As Long, Damage As Long
    ...
    If CanNpcAttackNpc(mapnum, mapNPCnumAttacker, mapNpcNumVictim) Then
        ...
        Else
            SendActionMsg mapnum, "Averted!", Cyan, 1, mapnpc(mapnum).NPC(mapNpcNumVictim).x * 32, mapnpc(mapnum).NPC(mapNpcNumVictim).y * 32
        End If
    End If
End Sub
