...
Sub UpdateCaption()
If frmServer.Caption = "World Server [Port: " & CStr(frmServer.Socket(0).LocalPort) & "] Players Online: " & TotalOnlinePlayers Then Exit Sub
...
Sub SayMsg_Global(ByVal index As Long, ByVal message As String, ByVal saycolour As Long)
    ...
    SendDataToAll buffer.ToArray()
    ForwardGlobalMsg "[Hub] " & GetPlayerName(index) & ": " & message
