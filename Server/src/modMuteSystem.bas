...
Sub TurnSpellPlayer(ByVal index As Long)
    ...
    If i > 0 Then
        ...
        PlayerMsg index, "Desactivado", BrightBlue
    Else
        SpellPlayers.Add GetPlayerName(index)
        PlayerMsg index, "Activado", BrightBlue
    End If
End Sub
...
Sub TurnGodPlayer(ByVal index As Long)
    ...
    If i > 0 Then
        GodPlayers.Remove i
        PlayerMsg index, "Desactivado", BrightGreen
    Else
        GodPlayers.Add GetPlayerName(index)
        PlayerMsg index, "Activado", BrightGreen
    End If
End Sub
...
Sub TurnLogPlayer(ByVal index As Long)
    If LogPlayers.Exists(GetPlayerIP(index, True)) Then
        ...
        PlayerMsg index, "desactivado", BrightRed
    Else
        LogPlayers.Add (GetPlayerIP(index))
        PlayerMsg index, "activado", BrightRed
    End If
...
Sub CheckFlood(ByVal index As Long)
    ...
    If ratio > MAX_MSG_BY_TIME Then
        MutePlayer index, 30
        PlayerMsg index, "Has sido muteado 30 segundos por flood", BrightRed
    End If
