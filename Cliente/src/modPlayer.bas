                                ...
                                If Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) - 1).Type = TILE_TYPE_DOOR Then
                                    If Player(index).PlayerDoors(Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) - 1).Data1).state = 0 Then
                                                ...
                                                If Doors(Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) - 1).Data1).UnlockType = 0 Then
                                                    PlayerMsg index, "Necesitas la llave correcta para abrir ésta puerta. (" & Trim$(Item(Doors(Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) - 1).Data1).key).Name) & ")", BrightRed
                                                ElseIf Doors(Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) - 1).Data1).UnlockType = 1 Then
                                                    PlayerMsg index, "Tienes que activar un interruptor para abrir ésta puerta.", BrightRed
                                                Else
                                ...
                                If Map(GetPlayerMap(index)).Tile(GetPlayerX(index) - 1, GetPlayerY(index)).Type = TILE_TYPE_DOOR Then
                                    If Player(index).PlayerDoors(Map(GetPlayerMap(index)).Tile(GetPlayerX(index) - 1, GetPlayerY(index)).Data1).state = 0 Then
                                                ...
                                                If Doors(Map(GetPlayerMap(index)).Tile(GetPlayerX(index) - 1, GetPlayerY(index)).Data1).UnlockType = 0 Then
                                                    PlayerMsg index, "Necesitas la llave correcta para abrir ésta puerta. (" & Trim$(Item(Doors(Map(GetPlayerMap(index)).Tile(GetPlayerX(index) - 1, GetPlayerY(index)).Data1).key).Name) & ")", BrightRed
                                ...
                                If Damage > 0 Then
                                        ...
                                        If ResourceCache(GetPlayerMap(index)).ResourceData(Resource_num).cur_health - Damage <= 0 Then
                                                ...
                                                ElseIf Resource(Resource_index).ItemSuccessMessage = True And Resource(Resource_index).Rewards(Reward_index).ItemReward > 0 Then
                                                        SendActionMsg GetPlayerMap(index), "¡" & Trim$(Item(Resource(Resource_index).Rewards(Reward_index).ItemReward).Name) & "!", BrightGreen, 1, (GetPlayerX(index) * 32), (GetPlayerY(index) * 32)
                                                End If
                                                ' carry on
...
Dim Exp As Long
    ...
    If Exp < 0 Then Exp = 0
        If Exp = 0 Then
            Call PlayerMsg(index, "No perdistes experiencia.", BrightRed)
        Else
            Call SetPlayerExp(index, GetPlayerExp(index) - Exp)
            SendEXP index
            Call PlayerMsg(index, "¡Has perdido " & Exp & " de experiencia!", BrightRed)
        End If
    ...
    If (GetPlayerInvItemNum(index, invNum) > 0) And (GetPlayerInvItemNum(index, invNum) <= MAX_ITEMS) Then
        ...
        Select Case Item(itemnum).Type
            Case ITEM_TYPE_ARMOR
                ...
                PlayerMsg index, "Te equipastes " & Item(itemnum).Name, BrightGreen
            ...
            Case ITEM_TYPE_WEAPON
                ...
                If Not CanPlayerEquipItem(index, Weapon) Then
                    Call PlayerMsg(index, "No puedes equiparte este item", BrightRed)
                    Exit Sub
                End If
            ...
            Case ITEM_TYPE_HELMET
                ...
                PlayerMsg index, "Te equipastes " & CheckGrammar(Item(itemnum).Name), BrightGreen
                TakeInvItem index, itemnum, 1
        ...
        Case ITEM_TYPE_TRIFORCE
            ...
            If Not GetPlayerLevel(index) >= MIN_LEVEL_TO_RESET Then
                PlayerMsg index, "Necesitar ser Lvl " & MIN_LEVEL_TO_RESET & " para adquirir la trifuerza", BrightRed
                Exit Sub
            End If
                ...
                If Player(index).PlayerDoors(Door_num).state = 0 Then
                    If Doors(Door_num).UnlockType = 0 Then
                        For i = 1 To MAX_INV
                            ...
                            If Doors(Door_num).key = key Then
                                ...
                                For n = 1 To Player_HighIndex
                                    If GetPlayerMap(index) = GetPlayerMap(n) Then
                                        ...
                                        PlayerMsg n, "Se ha desbloqueado algo", BrightBlue
                            ...
                            End If
                        Next
                        PlayerMsg index, "No posees la llave adecuada para abrir la puerta.", BrightBlue
                    ElseIf Doors(Door_num).UnlockType = 1 Then
                        If Doors(Door_num).state = 0 Then
                            PlayerMsg index, "No has pulsado el interruptor adecuado para abrir la puerta.", BrightBlue
                        End If
                    ElseIf Doors(Door_num).UnlockType = 2 Then
                        PlayerMsg index, "Ésta puerta no está cerrada.", BrightBlue
                    End If
                    
                Else
                    PlayerMsg index, "Ésta puerta ya está abierta.", BrightBlue
                End If
                ...
                'start new method: Access all players on map to the switch
                    For n = 1 To Player_HighIndex
                        If GetPlayerMap(index) = GetPlayerMap(n) Then
                            ...
                            SendPlayerData (n)
                            Call SendPlayerDoor(n, Doors(Door_num).Switch)
                            PlayerMsg n, "El interruptor ha sido activado", BrightBlue
                            SendMapSound index, GetPlayerX(index), GetPlayerY(index), SoundEntity.seSwitch, 1
                        End If
                    Next
                'end
                    ...
                    'Starting new method: same as previous sub methods
                        For n = 1 To Player_HighIndex
                            If GetPlayerMap(index) = GetPlayerMap(n) Then
                                    ...
                                    SendPlayerData (n)
                                    Call SendPlayerDoor(n, Doors(Door_num).Switch)
                                    PlayerMsg n, "El interruptor ha sido desactivado", BrightBlue
                                    SendMapSound index, GetPlayerX(index), GetPlayerY(index), SoundEntity.seSwitch, 1
                            End If
                        Next
                    'end
