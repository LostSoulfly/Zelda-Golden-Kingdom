...
If TempPlayer(index).targetType <> TARGET_TYPE_NPC Then
    Call PlayerMsg(index, "No tienes ningun NPC fijado", BrightRed)
    Exit Sub
End If
...
Sub CheckPlayerPetLevelUp(ByVal index As Long)
    ...
    Do While GetPlayerPetExp(index) >= GetPlayerPetNextLevel(index)
        ...
        PlayerMsg index, PointsWon & " puntos ganados!", Blue
...
Call PlayerMsg(index, "Abandonas a tu " & Trim$(NPC(Pet(Player(index).Pet(PetSlot).NumPet).NPCNum).Name) & " y recibes la mitad de su Exp: " & CumExp / 2 & " puntos!", Yellow)
Call ResetPlayerPetSlot(index, PetSlot)
